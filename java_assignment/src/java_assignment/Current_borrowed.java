...
public class Current_borrowed extends JFrame{
	...
	private void InitGUI() {
		...
		btnCurrentBorrowed.setBackground(new Color(96, 163, 217));
		btnCurrentBorrowed.setColor_white();
		...
		Object[] column = {"ID","Preview","Name", "Borrow Date", "Expiry Date" , "Renew chance"};
		Object[] row = new Object[0];
		model.setColumnIdentifiers(column);
		tblBorrowedBook.setModel(model);
		tblBorrowedBook.setRowHeight(300);
		...
		lblTitle = new JLabel("Borrowed Book");
		...
		lblFavourite.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				Favourite setFavourite = new Add_favourite(
						tblBorrowedBook.getValueAt(tblBorrowedBook.getSelectedRow(), 0).toString(),
						tblBorrowedBook.getValueAt(tblBorrowedBook.getSelectedRow(), 2).toString(),
						show_date
						);
