...
public class Favourite_book extends JFrame{
	...
	private JTable tblFavouriteBook;
	...
	public Favourite_book() {
		addWindowListener(new WindowAdapter() {
			...
			public void windowOpened(WindowEvent e) {
				   String filepath = "favourite_book.txt";
	...
	private void InitGUI() {
		...
		btnFavouriteBook.setBackground(new Color(96, 163, 217));
		btnFavouriteBook.setColor_white();
		...
		tblFavouriteBook = new JTable();
		tblFavouriteBook.setBorder(new EmptyBorder(1, 1, 1, 1));
		model = new Table_model();
		Object[] column = {"ID","Preview","Name","Type", "Status", "Favourite Date"};
		Object[] row = new Object[0];
		model.setColumnIdentifiers(column);
		tblFavouriteBook.setModel(model);
		tblFavouriteBook.setRowHeight(300);
		tblFavouriteBook.setFont(new Font("SansSerif", Font.PLAIN, 18));
		tblFavouriteBook.getColumn("Preview").setCellRenderer(new Pic_render());
		TableColumnModel columnModel = tblFavouriteBook.getColumnModel();
		...
		tblFavouriteBook.getTableHeader().setResizingAllowed(false);
		tblFavouriteBook.getTableHeader().setReorderingAllowed(false);
		tblFavouriteBook.getTableHeader().setFont(new Font("SansSerif", Font.PLAIN, 15));
		tblFavouriteBook.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		tblFavouriteBook.setAutoCreateRowSorter(true);
		tblFavouriteBook.getSelectionModel().addListSelectionListener(new ListSelectionListener(){
		...
		scrollPane.setViewportView(tblFavouriteBook);
		
		
		lblTitle = new JLabel("Favourite Book");
		...
		btnRemove.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				String idBook = tblFavouriteBook.getValueAt(tblFavouriteBook.getSelectedRow(), 0).toString();
				String bookName = tblFavouriteBook.getValueAt(tblFavouriteBook.getSelectedRow(), 2).toString();
				
				dispose();
				Remove_confirm ask = new Remove_confirm(idBook, bookName);
		...
		btnRemove.setFont(new Font("SansSerif", Font.PLAIN, 24));
		btnRemove.setBounds(707, 624, 281, 59);
		btnRemove.setBackground(new Color(0, 59, 115));
		btnRemove.setForeground(Color.white);
		pnContent.add(btnRemove);
		...
		txtSearchBook.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				tblFavouriteBook.setRowSorter(tr);
				tr.setRowFilter(RowFilter.regexFilter(search_text));
				
				try {
				tblFavouriteBook.setRowSelectionInterval(0, 0);
				}
				catch(Exception e1){
