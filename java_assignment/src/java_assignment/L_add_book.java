...
public class L_add_book extends JFrame{
	...
	private void InitGUI() {
		...
		lblTitle = new JLabel("Adding New Book");
		...
		btnAdd.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(txtBookID.getText().trim().isEmpty() && 
				...
				else if (imgUploaded == false){
					
					lblError.setText("**Please upload image for preview purpose");
					lblError.setVisible(true);
					
				}else if(txtBookID.getText().trim().isEmpty()) {
					
					lblError.setText("**Please fill in the Book ID");
					lblError.setVisible(true);
					...
					Check_book checkExist = new Check_book(txtBookID.getText().toString().toLowerCase());
					
					if(checkExist.getExist() == true) {
						lblError.setText("**The Book ID is existed in the database, please try again");
						lblError.setVisible(true);
					}
					else {
					
						//If does not exist, save image in the folder
						try {
						...
						//Then, add book details in the database
						Book newBook = new Add_book(
								txtBookID.getText().toString().toLowerCase(), imgPath,txtBookName.getText().toString(), 
								CBXbookType.getSelectedItem().toString(), "Available");
						
						dispose();
						
						//Call add book success window
						Add_success notification = new Add_success();
					}
				}
		...
		btnAdd.setFont(new Font("SansSerif", Font.PLAIN, 24));
		btnAdd.setBounds(480, 625, 411, 59);
		btnAdd.setBackground(new Color(0, 59, 115));
		...
		txtBookID.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent e) {
				lblError.setVisible(false);
			}
		...
		txtBookID.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e) {
				...
				char c = e.getKeyChar();
				if(Character.isSpaceChar(c) || !(Character.isLetterOrDigit(c)) || txtBookID.getText().trim().length() > 4){
					e.consume();
				}
			}
		});
		...
		txtBookID.setBackground(new Color(191, 215, 237));
		...
		CBXbookType.setModel(new DefaultComboBoxModel<Object>(new String[] {"- Select Book Type -", "Fiction", "Mystery", "Novel", "Horror", "Fantasy", "Romance"}));
